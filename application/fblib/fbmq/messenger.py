...
import json
from application.fblib.fbmq import Attachment, Template, QuickReply, NotificationType
from application.controllers.fbpage import page, CONFIG
...
def received_message(event):
    ...
    if message_text:
        ...
        page.send(sender_id, message_attachments)
...
def send_image(recipient, attachment):
    page.send(recipient, attachment, metadata="DEVELOPER_DEFINED_METADATA")


def send_gif(recipient, url):
    page.send(recipient, Attachment.Image(url))


def send_audio(recipient, url):
    page.send(recipient, Attachment.Audio(url))


def send_video(recipient, url):
    page.send(recipient, Attachment.Video(url))


def send_file(recipient, path):
    page.send(recipient, Attachment.File(path))
...
def send_text_message(recipient, text):
    return page.send(recipient, text, metadata="DEVELOPER_DEFINED_METADATA")

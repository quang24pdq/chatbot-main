define(function (require) {
	...
	var template = require('text!./tpl/broadcast-generic-template.html');
	...
	return Gonrin.ModelView.extend({
		...
		render: function () {
			...
			if (elements[0].title) {
				self.$el.find("#title").val(elements[0].title);
			}
			if (elements[0].subtitle) {
				self.$el.find("#subtitle").val(elements[0].subtitle);
			}
		...
		eventRegister: function () {
						...
						$.ajax({
							...
							success: function (dataBot) {
								...
								$.ajax({
									...
									success: (response) => {
										spinner.hide();
										elements[0].image_url = "https://www.facebook.com/photo.php?fbid=" + response.id;
										self.model.set('elements', elements);
										self.trigger('change', self.model.toJSON());
									},
									error: (error) => {
			...
			this.$el.find("#title").keyup(function () {
				var elements = self.model.get('elements');
				var value = this.value;
				elements[0].title = value;
				self.model.set("elements", elements);
				console.log("model generic : ", self.model)
				self.trigger('change', self.model.toJSON());
			});
			this.$el.find("#subtitle").keyup(function () {
				var elements = self.model.get('elements');
				var value = this.value;
				elements[0].subtitle = value;
				self.model.set("elements", elements);
				self.trigger('change', self.model.toJSON());
			});
